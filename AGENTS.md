...
- The `go` pin (in `.config/mise.toml` and the `go.mod` directive) gets its own PR and a manual merge: TinyGo refuses any GOROOT newer than the Go release it supports, and a mismatch fails `build-wasm` and `test-integration`. TinyGo 0.42.0 raised that ceiling to Go 1.27, so the `allowedVersions` cap that held the pin below 1.27 is gone; if a future Go minor outruns the pinned TinyGo, bump TinyGo first or re-add the cap.
- `gomodTidy` runs after every `go.mod` update, so Renovate branches keep `go.sum` tidy on their own.
- The config extends `config:best-practices`, which adds digest pinning for actions and the devcontainer image, config-migration PRs when Renovate deprecates syntax we use, and abandonment warnings on the dashboard. Its npm-only pieces are inert here.
- The Dependency Dashboard issue autocloses when there are no update branches and no deprecation warnings, and Renovate reopens the same issue (keeping its number, and so its pin) as soon as there is something to show. Closing it by hand does not disable it; that needs `dependencyDashboard: false`.
